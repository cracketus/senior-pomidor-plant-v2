...
When present, `system_health.watchdog.configured` distinguishes an intentionally absent optional host watchdog (`false`) from an installed watchdog whose status is unavailable or stale (`true`). Installation intent comes from `data/watchdog/installed`, with an existing status file retained as a backward-compatible signal. An absent watchdog and a systemd manager that cannot be accessed from the telemetry container are neutral to aggregate health; a configured stale watchdog or an observable inactive systemd service degrades it.
